...
int main(int argc, char **argv)
{
    ...
    Telemetry::Result set_rate_position_result = telemetry->set_rate_position(1.0);
    if (set_rate_position_result != Telemetry::Result::SUCCESS) {
        std::cout << ERROR_CONSOLE_TEXT << "Setting rate failed:" << Telemetry::result_str(
                      set_rate_position_result) << NORMAL_CONSOLE_TEXT << std::endl;
        return 1;
    }
    // We want to listen to the altitude of the drone at 10 Hz.
    Telemetry::Result set_rate_attitude_result = telemetry->set_rate_attitude(10.0);
    if (set_rate_attitude_result != Telemetry::Result::SUCCESS) {
        std::cout << ERROR_CONSOLE_TEXT << "Setting rate failed:" << Telemetry::result_str(
                      set_rate_attitude_result) << NORMAL_CONSOLE_TEXT << std::endl;
        return 1;
    }
    ...
    telemetry->attitude_euler_angle_async([](Telemetry::EulerAngle attitude) {
        ::fprintf(stderr,
                  TELEMETRY_CONSOLE_TEXT "Attitude: %f %f %f\n",
                  attitude.roll_deg,
                  attitude.pitch_deg,
                  attitude.yaw_deg);
    });
    ...
    while (true) {
        if (telemetry->health_all_ok() == true) {
            std::cout << "Vehicle is ready to arm" << std::endl;
        }
        sleep_for(seconds(1));
    }
